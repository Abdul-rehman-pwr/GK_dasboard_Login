...
export const getBackgroundColor = ({ layout, color, themeColor, invoiceText }) =>
  layout === 'Straight'
    ? 'white'
    : layout === 'Circle'
      ? color === 'default'
      ...
      : layout === 'Rounded'
        ...
        : layout === 'Basic' || layout === 'Straight'
          ...
          : invoiceText === 'default'
            ? '#1671C3'
            : themeColor
export const getInvoiceTextColor = styles => {
  if (styles?.invoiceText === 'default') return '#1671C3'
  if (['Rounded', 'Basic'].includes(styles?.layout)) return styles?.invoiceText
  return styles?.color || styles?.themeColor
}
