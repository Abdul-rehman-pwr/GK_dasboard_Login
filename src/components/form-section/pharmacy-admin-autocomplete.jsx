...
const PharmacyAdminAutocompleteField = ({ value, onChange, error, field, disabled }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true)
      try {
        const res = await pharmacyAdminServices.getAllPharmacyAdminsWithIdAndNames()
        if (res?.status == 200) {
...
export default PharmacyAdminAutocompleteField
